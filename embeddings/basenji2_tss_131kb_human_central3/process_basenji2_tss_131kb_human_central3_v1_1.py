...
EMBEDDING_NAME = "Basenji2-TSS-131kb-human-central3-log1p-zscore-v1.1"
SEQ_LEN = 131_072
...
def one_hot_dna_channels_first(seq: str) -> np.ndarray:
    arr = np.frombuffer(seq.upper().encode("ascii"), dtype=np.uint8)
    x = np.zeros((4, len(arr)), dtype=np.float32)
    x[0, arr == ord("A")] = 1.0
    x[1, arr == ord("C")] = 1.0
    x[2, arr == ord("G")] = 1.0
    x[3, arr == ord("T")] = 1.0
...
def main() -> None:
    ...
    ap.add_argument(
        "--outdir",
        default=str(Path.home() / "data/processed_embeddings/basenji2_tss_131kb_human_central3_v1_1"),
    )
    ap.add_argument(
        "--report_dir",
        default=str(Path.home() / "reports/mapping_reports/basenji2_tss_131kb_human_central3_v1_1"),
    )
    ...
    if genes.empty:
        raise RuntimeError("No valid genes for Basenji2.")

    genes.to_csv(outdir / "genes.tsv", sep="\t", index=False)
    skipped_df.to_csv(report_dir / "skipped_tss_windows.tsv", sep="\t", index=False)

    n = len(genes)
    raw_path = outdir / "raw_basenji2_human_central3_mean_tracks.npy"
    log_path = outdir / "raw_basenji2_human_central3_log1p_tracks.npy"
    ...
    print("Loading Basenji2...")
    model = Basenji2(basenji2_params["model"])
    weights = basenji2_weights()
    state = torch.load(weights, map_location="cpu")
    model.load_state_dict(state)
    model.eval()
    model.to(device)
    print("Loaded Basenji2.")
    ...
    with torch.no_grad():
        for batch_start in tqdm(range(0, len(todo), args.batch_size)):
            ...
            for idx in idxs:
                ...
                batch.append(one_hot_dna_channels_first(seq))
            ...
            if isinstance(y, dict):
                if "human" in y:
                    ...
                    raise RuntimeError(f"Model returned dict without human key: {list(y.keys())}")

            y = y.detach().cpu().numpy().astype(np.float32)

            if y.shape[1:] != (TARGET_LENGTH, N_TRACKS_HUMAN):
                raise RuntimeError(f"Unexpected Basenji2 output shape: {y.shape}")

            central3 = y[:, CENTRAL_SLICE, :].mean(axis=1)
            raw[idxs, :] = central3
            done[idxs] = True
    ...
    if not np.isfinite(raw_arr).all():
        raise RuntimeError("Raw Basenji2 matrix contains non-finite values.")
    ...
    pd.DataFrame(
        {
            ...
            "feature_name": [f"basenji2_human_track_{i}" for i in range(N_TRACKS_HUMAN)],
        }
    ).to_csv(outdir / "basenji2_human_track_columns.tsv", sep="\t", index=False)

    metadata = {
        "embedding_name": EMBEDDING_NAME,
        "status": "processed_qc_passed",
        "modality": "regulatory DNA / Basenji2 TSS sequence model",
        "model": "Basenji2 PyTorch port",
        "weights": str(weights),
        "input_tensor_layout": "channels_first, shape = batch x 4 x 131072",
        "output_shape_per_sequence": [TARGET_LENGTH, N_TRACKS_HUMAN],
        "pooled_representation": "mean of central 3 output bins around TSS",
        "raw_output_file": "raw_basenji2_human_central3_mean_tracks.npy",
        ...
        "input_sequence_type": "GRCh38 genomic DNA, 131072 bp centered on gene TSS",
        ...
        "boundary_handling": "N-padding if the 131072 bp window extends beyond chromosome boundaries",
...
Raw Basenji2 human central3 matrix:
shape: {raw_arr.shape}
dtype: {raw_arr.dtype}
...
- 131072 bp sequence centered on TSS.
...
- Used Basenji2 human-track output: 5313 tracks.
- Averaged central 3 output bins around the TSS.
- Applied log1p to raw track predictions.
